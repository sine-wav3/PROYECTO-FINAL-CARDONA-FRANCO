...
Elija una opcion: """) #opciones que redirigen al usuario hacia la funcion enumerada que prefiera
            ...
            print(f"Ha ocurrido un error: {e}. Intente nuevamente.") #excepcion principal de todas las funciones, 
            # si falla algo se redirige al menu.
...
def create_students():
    ...
    while True:
        try:
            ...
            if not name.replace(" ", "").isalpha(): #metodo .isalpha para no permitir que el usuario 
                # ingrese un tipo de dato string
    ...
    while True:
        ...
        if not code_students.isdigit(): #metodo .isdigit para confirmar que el codigo si es numerico
            print("El código solo debe contener números. Intente nuevamente.")
            continue
        if any(est["code_students"] == code_students for est in students):
            print("Ya existe un estudiante con este código.")
            return
        break
    ...
    grades = {}
    for code_subject, data_subject in subjects.items(): #se accede a los items del diccionario 
        # a traves de los iteradores para los valores de las llaves
        while True:
            try:
                try:
...
def delete():
    print("\n--- Eliminar Estudiante ---")
    while True:
        code_students = input("Ingrese el código del estudiante que desea eliminar (solo números): ")
        if not code_students.isdigit():
            print("El código solo debe contener números. Intente nuevamente.")
            continue
        break

    student = next((est for est in students if est["code_students"] == code_students), None) #usa un generador 
    # para buscar dentro de la lista de los estudiantes
        ...
        if option == "1":
            while True:
                ...
                if not code_students.isdigit():
                    ...
                    continue
                break
            student = next((est for est in students if est["code_students"] == code_students), None) #uso de generador 
            #para buscar el valor que coincida con lo almacenado dentro de estudiantes
...
#funcion para cargar datos desde un archivo .txt         
def load_students_from_file():
    print("\n--- Cargar Estudiantes desde Archivo ---")
    file_path = input("Ingrese la ruta del archivo .txt: ")
    try:
        with open(file_path, 'r', encoding='utf-8') as file: # con enconding se asegura que se pueda desencriptar 
            # el archivo correctamente
            for line in file:
                data = line.strip().split(',') #se quitan las comas que separan las llaves y los valores del archivo 
                # "NotasEstudiantesDeIngenieria2"
...
def filter_students():
    ...
    if option == "1": # ordenar por promedio ponderado
        averages = []
        for student in students:
            # Calcular la nota total multiplicada por lo creditos de cada asignatura
            total_points = sum(student["grades"][code_subject] * data_subject["credits"] for code_subject, data_subject in subjects.items())
            total_credits = sum(data_subject["credits"] for data_subject in subjects.values())
        ...
        # reverse=True usado para organizar la lista desde el estudiante con el mayor promedio hasta el de menor promedio
        # key=lambda usado como funcion que retorna un valor (en este caso retorna el mayor promedio dentro de average)
        # para clasificar y ordenar usando su indice como clave
...
if __name__ == "__main__": # entry point para almacenar la funcion principal y a partir de esta acceder a las auxiliares
    main()